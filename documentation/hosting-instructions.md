...
- ssh into the server
    - For Mac and Linux: `ssh root@mythwovenmaille.com`
    - For Windows: Set up a new PuTTY session
        ...
        - **Session**
            ...
            - Click Save
- create user
    - `useradd -m -s /bin/bash -g users -G sudo jkuhn`
    - replace jkuhn with your own
- create password
    - `passwd jkuhn`
- Move .ssh key file
    - `cp -r .ssh /home/jkuhn`
- change ownership of the .ssh directory
    - `cd /home/jkuhn`
    - `chown -R jkuhn:users .ssh/`


- Test Changes
    - `exit`
    - Mac/Linux: `ssh jkuhn@mythwovenmaille.com`
    - Windows:
        - **Session**
            - Click pwp
            - Click Load
...
- create a directory for your deployment
    - `sudo mkdir /var/www/mythwovenmaille.com/html`
- fix permissions
    - `sudo chown -R $USER:users /var/www/mythwovenmaille.com/html`
...
- `sudo vim /etc/nginx/sites-available/mythwovenmaille.com`
...
server {
    ...
    root /var/www/mythwovenmaille.com/html;
    index index.html index.htm index.nginx-debian.html;

    server_name mythwovenmaille.com www.mythwovenmaille.com;
...
- create a symlink to the sites-enabled directory
    - `sudo ln -s /etc/nginx/sites-available/mythwovenmaille.com /etc/nginx/sites-enabled/`
...
- Create a new ssh key on your server in the .ssh directory
    - `ssh-keygen -t ed25519 -C  "github Deploy Key" -f /home/jkuhn/.ssh/github_deploy_key`
    - Don't enter a passphrase
        - hit enter twice
- Copy the public key into your authorized keys file
    - `cat /home/jkuhn/.ssh/github_deploy_key.pub >> /home/jkuhn/.ssh/authorized_keys`
...
- Copy the private key to your clipboard **(this is a very dangerous operation do not share this key with anyone )**
    ...
    - `cat /home/jkuhn/.ssh/github_deploy_key`
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to server
        run: |
         ssh-keyscan -H mythwovenmaille.com >> ~/.ssh/known_hosts 
         scp -r ./dist/* gkephart@mythwovenmaille.com:/var/www/mythwovenmaille.com/html
```
